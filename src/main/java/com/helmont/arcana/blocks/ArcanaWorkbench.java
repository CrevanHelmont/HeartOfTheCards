...
public class ArcanaWorkbench extends Block {
    public static final Text TITLE = Text.translatable("container.arcana_workbench");
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        if (checkForCandles(world, pos)) {
            int npx = random.nextBetweenExclusive(0, 2);
            int npy = random.nextBetweenExclusive(0, 2);
            int npz = random.nextBetweenExclusive(0, 2);
            if (npx == 0) {
                npx = 1;
            } else {
                npx = -1;
